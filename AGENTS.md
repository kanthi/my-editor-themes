...
Dark variants of Midnight Aurora live next to the Subtle theme and use the official `"include"` field. Override `colors`, `semanticTokenColors`, and the full `tokenColors` array (same scopes as Subtle, remapped from that variant's palette). Language-specific TextMate rules in Subtle would otherwise keep Subtle's hex. Do not drop `include` or duplicate the VS Code port into `ports/cursor` / `ports/antigravity-ide`.
